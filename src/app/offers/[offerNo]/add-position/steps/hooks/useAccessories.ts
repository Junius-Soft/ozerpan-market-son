  ...
  useEffect(() => {
    const fetchAndCalculateAccessories = async () => {
      try {
        ...
        const allAccessories: Accessory[] = data.productAccessories.panjur;
          ...
          if (selections.movementType === "manuel") {
            if (selections.manuelSekli === "makarali") {
              ...
              const reduktor = allAccessories.find((acc) =>
                acc.description.toLowerCase().includes("panjur redüktörü beyaz")
              );
              if (reduktor) {
        ...
        if (
          ...
          if (celikAski) {
            ...
            if (askiTeli) {
              const askiTeliAccessory = { ...askiTeli, quantity: askiQuantity };
