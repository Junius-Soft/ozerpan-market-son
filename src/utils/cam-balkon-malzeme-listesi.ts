...
export function calculateAksesuarMalzemeleri(
  ...
  const toplamCikisSayisi = kolBilgileri.reduce((sum, kol) => sum + (Number(kol.cikis_sayisi) || 0), 0);

  // Hareketli cam sayısını hesapla (toplam kanat - sabit cam - çıkış camı)
  // NOT: Sabit camlarda (sabitCamAdedi) tekerlek olmaz!
  ...
  const toplamHareketliCam = kolBilgileri.reduce((sum, kol) => {
    const kanatSayisi = Number(kol.kanat) || 0;
    const sabitSayisi = Number(kol.sabitCamAdedi) || 0;
    const cikisSayisi = Number(kol.cikis_sayisi) || 0;

    // Sadece gerçek sürme (sliding) camları say
    const hareketliCamSayisi = Math.max(0, kanatSayisi - sabitSayisi - cikisSayisi);

    console.log(`🔍 Kol tekerlek hesabı: Kanat: ${kanatSayisi}, Sabit: ${sabitSayisi}, Çıkış: ${cikisSayisi} -> Hareketli: ${hareketliCamSayisi}`);
